...
__global__ void Sgemm(
  ...
  const int STRIDE_A = BLOCK_SIZE_M / (THREAD_SIZE_Y / 4);
  const int STRIDE_B = BLOCK_SIZE_N / (THREAD_SIZE_X / 4);
  ...
  for (int k = 0; k < K; k += BLOCK_SIZE_K) {
    ...
    // bank conflict
    //   在TM = TN = 8的情况下，无论矩阵A还是矩阵B，从Shared Memory中取数时需要取连续的8个数，
    //   即便用LDS.128指令一条指令取四个数，也需要两条指令，由于一个线程的两条load指令的地址是连续的，
    //   那么同一个Warp不同线程的同一条load指令的访存地址就是被间隔开的，便存在着Bank Conflict。
    #pragma unroll
    for (int i = 0; i < BLOCK_SIZE_K; ++i) {
      #pragma unroll
      for (int j = 0; j < THREAD_SIZE_Y; j += 4) {
        FETCH_FLOAT4(frag_a[j]) = FETCH_FLOAT4(smemA[i][ty * 4 + (j / 4) * STRIDE_A]);
      }
      #pragma unroll
      for (int j = 0; j < THREAD_SIZE_X; j += 4) {
        FETCH_FLOAT4(frag_b[j]) = FETCH_FLOAT4(smemB[i][tx * 4 + (j / 4) * STRIDE_B]);
      }
      #pragma unroll
...
#pragma unroll
    for (int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x += 4) {
      FETCH_FLOAT4(C[OFFSET(by * BLOCK_SIZE_M + ty * 4 + thread_y % 4 + (thread_y / 4) * STRIDE_A,
                            bx * BLOCK_SIZE_N + tx * 4 + (thread_x / 4) * STRIDE_B,
                            N)]) = FETCH_FLOAT4(accum[thread_y][thread_x]);
    }
  }
